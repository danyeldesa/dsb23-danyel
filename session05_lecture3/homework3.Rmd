...
```{r}
#first we pull the payments and members tables from sky_westminster
payments <- dplyr::tbl(sky_westminster, "payments")
members <- dplyr::tbl(sky_westminster, "members")

#left join the tables, mataching member_id from payments with id from members
payments_members <- left_join (payments, members,by=c("member_id"="id"))

MP_money <- payments_members %>% 
  #first we collect all data since we are pulling from a database online
  collect() %>%
  #group by name of MP
  group_by(name) %>% 
  #create new variable total which sums up the donations that MP has received
...
We see Theresa May has received the highest donations with £2.8Mn
...
```{r}
donations_by_entity <- payments_members %>% 
  # first collect since we are pulling from an online database
  collect() %>% 
  # calculate the total donations across all entities
  summarise(entity, value,total_donations=sum(value)) %>% 
  # group by entity and find out what % of the total they donated
  group_by(entity) %>%
  summarise(entity_donation=sum(value),percent=entity_donation/total_donations*100) %>% 
  distinct() %>% 
...
```{r}
single_party_donors <- payments_members %>% 
  #first collect
  collect() %>% 
  #calculate total number of donors
  #group by entity and check if the number of parties donated to is more than one. We can do this by seeing the length of list of unique parties donated to.
  group_by(entity) %>% 
  summarise(single_party=(length(unique(party_id))==1)) %>%
  distinct() %>% 
  #calculate the total number of unique donors
  summarise (single_party, total_donors=n()) %>% 
  group_by (single_party) %>% 
  summarise(number_donors=n(),
            total_donors, 
            percent=number_donors/total_donors) %>% 
  distinct()

single_party_donors
```
We see from the table above that the total number of entities donating is 2,213. The number of single party-donors is 2,036 or 92% if the total, and those donating to more than one party is 177 or 8% of the total.
...
```{r}
donations_party_year <- payments_members %>% 
  collect() %>% 
  # convert year to int
  mutate(year=as.numeric(substr(date,nchar(date)-4,nchar(date)))) %>% 
  group_by(year) %>%
  summarise(value,total_annual_donation=sum(value),party_id) %>% 
  # group by all variables other than donation
  group_by(year,party_id) %>% 
  summarise(total_party_donation=sum(value),
            prop=round(total_party_donation/total_annual_donation,5),
            party_id) %>% 
  distinct()
  
# join the tables
donations_party_year <- left_join(donations_party_year, 
                                  parties, 
                                  by=c("party_id"="id"),
                                  copy=TRUE)

output_table <- donations_party_year %>%
  ungroup() %>% 
  select(year, name, total_party_donation, prop) %>% 
  filter(!year==2019) %>% 
  arrange(year)
...
```
```{r}
data_collected <- cdc_data %>%
  # group by all categories that we need
...
```
```{r}
chart_data <- data_collected %>% 
  # filter for only parameters we want  
  filter(!age_group %in% c("Missing", NA),
           sex %in% c("Female", "Male"),
           icu_yn %in% c("No", "Yes")) %>%
    group_by (age_group, sex, icu_yn) %>%   
  # calculate CFR
    summarise(deaths = sum(cases [death_yn == "Yes"], na.rm = TRUE),
              total_cases = sum(cases),
              cfr = round(100 * deaths/total_cases, 0)) %>% 
  # change the ICU column
  mutate(icu_yn = case_when(
      icu_yn == "Yes" ~ "ICU Admission",
      icu_yn == "No" ~ "No ICU Admission"))


ggplot(chart_data, aes(y=cfr, x=age_group, fill="orange")) +
  geom_col(show.legend =FALSE) + 
  coord_flip() + 
  theme_light(base_size = 5) +
  labs(x = NULL, y = NULL, title = "Covid CFR % by age group, sex and ICU Admission") +
  # add text
  geom_text(aes(label = cfr, hjust = 1.2)) +
  # facet wrap by ICU admission and gender
...
ggplot(chart_data_2, aes(x=case_month, y=cfr, color = age_group, group = age_group)) +
  geom_line() +
  ...
  labs(x = NULL, y = NULL, title = "Covid CFR % by age group, sex and ICU Admission") +
  geom_text(data =.%>% filter(cfr != 0), aes(label = cfr, hjust = -0.4),
            size = 2) +
  facet_grid(rows = vars(icu_yn), cols = vars(sex)) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))
...
tables_extracted <- contributions_tables %>% 
  html_nodes(css = "table") %>% 
  html_table()
  
contributions <- tables_extracted[[1]] %>% 
    janitor::clean_names()
...
scrape_pac <- function(x) {
  ...
  # use the code above to split country/parent and convert currencies
  contributions <- contributions %>%
    separate(country_of_origin_parent_company, 
           into = c("country", "parent"), 
           sep = "/", 
...
-   Who did you collaborate with: Danyel Desa
-   Approximately how much time did you spend on this problem set: 4 hours
-   What, if anything, gave you the most trouble: Length of assignment
